...
def extract(transition):
    ...
    gripper_qpos[-1] = -1 * gripper_qpos[-1]
            ...
            while t < max_steps:
                try:    
                    ...
                    if args.use_reticle:
                        ...
                        front_rgb_reticle, wrist_rgb_reticle = reticle_builder.render_on_front_and_wst_camera(
                            front_camera_rgb=front_rgb,
                            front_camera_depth=front_depth,
                            front_camera_extrinsics=np.linalg.inv(front_camera_extrinsics),
                            front_camera_intrinsics=front_camera_intrinsics,
                            wrist_camera_rgb=wrist_rgb,
                            wrist_camera_depth=wrist_depth,
                            wrist_camera_extrinsics=np.linalg.inv(wrist_camera_extrinsics),
                            wrist_camera_intrinsics=wrist_camera_intrinsics,
                            gripper_pos=rlbench_state[:3],
                    ...
                    if rlbench_state[-1] == 1: # gripper open
                        joint_state = np.concatenate([joint_state, [0.0]], axis=-1)
                    else:
                        joint_state = np.concatenate([joint_state, [1.0]], axis=-1)
                    ...
                    joint_pos_action = action_plan.popleft()
                    print(f"Action: {joint_pos_action}")

                    # Execute action in environment
                    if joint_pos_action[-1] == 1: # close
                        joint_pos_action[-1] = 0 # RLbench needs gripper open state
                    else:
                        joint_pos_action[-1] = 1

                    transition = env.step(joint_pos_action)
